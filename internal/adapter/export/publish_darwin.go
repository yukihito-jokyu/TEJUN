...
func exchangeChecked(root *os.Root, staging, target string, want application.OverwriteIdentity) error {
	...
	fd := int(directory.Fd())
	if err := unix.RenameatxNp(fd, staging, fd, target, unix.RENAME_SWAP); err != nil {
		return err
	}

	old, err := rootIdentity(root, staging)
	if err != nil || !sameIdentity(old, &want) {
		swapBackErr := unix.RenameatxNp(fd, staging, fd, target, unix.RENAME_SWAP)

		return errors.Join(&shared.Error{
			Code: "export_source_changed", Message: "上書き対象が変更されました", Retryable: true,
		}, swapBackErr)
	}
